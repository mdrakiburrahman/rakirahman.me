...
In short, because Fabric Spark offers the exact same API as local Spark, and **locally reproducible test coverage is the _most important capability_ in leveraging AI models to write your code**.

The most important rule of Continuous Integration testing is, it needs to be 100% deterministic with zero room for flakiness - see this blog from Datadog - [Flaky tests: their hidden costs and how to address flaky behavior](https://www.datadoghq.com/blog/datadog-flaky-tests/).

This is why I did not go with the [Fabric Data Warehouse adapter](https://docs.getdbt.com/docs/core/connect-data-platform/fabric-setup) after evaluating it, you need a Fabric Workspace to run your tests in GitHub Action/Azure DevOps Pipelines.
For 1000 parallelized AI-generated PRs, you'll need to spin up 1000 Fabric Workspaces, 1000 capacities, and potentially, 1000 support requests for subscription quota increase.
...
[Snowflake](https://docs.snowflake.com/en/) also has this problem despite being the granddaddy of Modern Data Warehouses that dbt has arguably the most adoption with - so someone built this thing called [fakesnow](https://github.com/tekumara/fakesnow) to simulate Snowflake locally. I bet the author wishes Snowflake could just release a Docker Container so they could be done with this problem without mocking the Snowflake SQL surface area.

<Callout>

In the age of AI-generated PRs on the rise, all of these proprietary cloud warehouse vendors will realize one day that you really must offer a lightweight Docker Container to enable local dev/testing that significantly drives product adoption. This is the only reason I use Spark nowadays, I can run the entire API surface area locally to test my business logic confidently.

Other giants have learnt this the hard way and done it, [SQL Server Container](https://hub.docker.com/r/microsoft/mssql-server), [Event Hub Container](https://hub.docker.com/r/microsoft/azure-messaging-eventhubs-emulator), [Cosmos DB Container](https://github.com/Azure/azure-cosmos-db-emulator-docker).

Zero dependency local dev/testing solves an **extremely real problem** at producing high-velocity, high-quality software.
...
That being said, if you're not on a team that's looking to milk the living heck out of AI Agents to generate code as parallely as possible on a large team, in my opinion, you should totally go with Fabric Warehouse for your ELT since it comes with less fancy knobs and less room for headaches.
The day Fabric Warehouse can offer a locally mockable Docker container that works with OneLake mounted data like Spark does, I'll probably start adopting it for ELT too, because the [Engine](https://learn.microsoft.com/en-us/fabric/data-warehouse/architecture) is super impressive and fast.
