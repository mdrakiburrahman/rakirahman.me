...
Hopefully stateful Activator queries that run off KQL has managed state.

We can always use Spark Streaming etc to avoid this problem (since it stores state in checkpoints), but that requires managing yet another SPOF thing.

### Aesthetic

The only other criticism I have is I can't embed more emojis into the Fabric Activator Teams Alert 😡.
