...
- We can [`patch`](https://kubernetes.io/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/) the K8s LB at any time in the process - because what matters is the SPN entry matches the ports.
...
- Currently, Active Directory works for the Data Controller in _Indirect_ mode, but most of the plumbing for _Direct_ mode seems to be in place for an upcoming release.
...
6. Keytab creation on Linux machine
...
We can perform the rest of the work from VS Code inside `client` - starting with deploying the Data Controller in [Indirect Mode](https://docs.microsoft.com/en-us/azure/azure-arc/data/create-data-controller-indirect-cli?tabs=windows):
...
First thing I tried to do is see if the script will work inside an Ubuntu 20.04 docker container ([here's the `dockerfile`](https://github.com/mdrakiburrahman/arcsqlmi-activedirectory/blob/main/keytab-docker/dockerfile))- however I found that the script as-written (uses [`printf`](https://github.com/microsoft/azure_arc/blob/43d971cca0a00756a4973be17c56236396d5e705/arc_data_services/deploy/scripts/create-sql-keytab.sh#L176) to call `ktutil` interactive prompts in stages - like [this suggestion from StackOverflow](https://stackoverflow.com/a/37454886)) doesn't work inside a container. I was running into `cannot read password while adding new entry` inside the container. As a workaround, I found the `echo` and `sleep` syntax to wrap `ktutil` as [suggested here](https://stackoverflow.com/a/69653057) works inside Docker, but didn't want to refactor the existing script into this brittle solution.

I _think_ this is because Containers are missing [`/dev/tty`](https://github.com/node-red/node-red-docker/issues/15) on Docker Desktop in WSL - which makes the `printf` wrapper above break.

So instead, I found the script works fine inside Ubuntu 20.04 VM spun up by [`multipass`](https://multipass.run/docs/installing-on-windows).
...
Here are the 3 YAML files we need to deploy:
...
2 - `sql-ad-yes-1-keytab-secret.yaml`: [docs](https://docs.microsoft.com/en-us/azure/azure-arc/data/deploy-active-directory-sql-managed-instance#deploy-kubernetes-secret-for-the-keytab)
...
3 - `sql-ad-yes-1.yaml`: [docs](https://docs.microsoft.com/en-us/azure/azure-arc/data/deploy-active-directory-sql-managed-instance#prepare-sql-managed-instance-spec)
...
We deploy the 3 YAMLs in sequence:
...
# Deploy keytab secret
kubectl apply -f sql-ad-yes-1-keytab-secret.yaml
...
I noticed that for `LoadBalancer` deploments, the `31433` specified in the spec above doesn't make it into the K8s Service - rather it shows up as `1433`
(see screenshot below.) As a workaround, I added the `kubectl patch svc...` to change the Port to `31433` - which is what our `SPN` expects.

For `NodePort` - this worked fine (i.e. `service` was created exposing `31433`.)
...
![Windows login creation in SQL MI](images/windows-login-creation.png)
