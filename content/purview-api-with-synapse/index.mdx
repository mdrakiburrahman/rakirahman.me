...
💡 In this article - we explore a PySpark script written in Synapse with some reusable functions to programmatically access Purview's REST API.
...
- And in return, we can say to **Synapse**: _"Go ahead and use Spark Pools to drop, obfuscate, deanonymize or otherwise deal with those columns"_
...
❗ These techniques/tools are from some initial exploration of the Purview REST API - shared here for exploratory/education purposes. For official Microsoft content - please refer to the [Purview Documentation](https://docs.microsoft.com/en-us/azure/purview/).
...
💡 We'll also be recursing through our Data Lake to retrieve file names using the techniques outlined in [this article](../directory-recursion-synapse/).
...
Next, we leverage PyApacheAtlas to return an `AtlasClient` using `atlas_auth`:

```Python
def atlas_auth(tenant_id: str, client_id: str, client_secret: str, atlas_endpoint: str):
    """
...
We use `get_all_adls_assets` to retrieve all scanned assets from our Data Lake from the Purview REST API:

```Python
def get_all_adls_assets(data_catalog_name: str, storage_account: str, container: str, azuread_access_token: str):
    """
    Retrieves all scanned assets for the specified Storage Account Container
    """
    url = f"https://{data_catalog_name}.catalog.purview.azure.com/api/browse"

    payload="""{"limit": 100,
                "offset": null,
                "path": "/azure_storage_account#%s.core.windows.net/azure_datalake_gen2_service#%s.dfs.core.windows.net/azure_datalake_gen2_filesystem#%s"
                }""" % (storage_account, storage_account, container)
    headers = {
        'Authorization': f'Bearer {azuread_access_token}',
        'Content-Type': 'application/json'
    }
    response = requests.request("POST", url, headers=headers, data=payload)
    assets_all = json.loads(response.text)
...
```python
def get_adls_asset_schema(assets_all: dict, asset: str, atlas_client: str):
    """
    Returns the asset schema and classifications from Purview
    """
    # Filter response for our asset of interest
    assets_list = jmespath.search(f"value[?name=='{asset}']", assets_all)
    ...
    for entry in assets_list:
        ...
        response = atlas_client.get_entity(entry['id'])
