...
One other point to call out is that Microsoft recently introduced the new Operating System [CBL-Mariner](https://en.wikipedia.org/wiki/CBL-Mariner) (**C**ommon **B**ase **L**inux) - which is used across the Microsoft ecosystem (not just Arc), such as Azure Stack HCI, AKS, IoT Edge and WSL. Source code is [available in Github here](https://github.com/microsoft/CBL-Mariner) - meaning anyone can transparently build an application on top of the Mariner OS - as I'll be demonstrating. 

As we'll see shortly, it appears that the vast majority our Arc containers above seem to leverage Mariner as the [Base image](https://docs.docker.com/develop/develop-images/baseimages/). 

## POC: Reverse Engineering a simple Mariner Container
...
As we can see, it's identical to the Dockerfile used to initially generate the image. We can use this technique to reverse engineer _any other_ Container Image we like as well (not just Mariner based) - such as the SQL MI pod that gets deployed for Azure Arc.
...
As we noted in our table earlier, the SQL MI "Pod" is actually made up of 3 containers: `arc-sqlmi`, `collectd` and `fluentbit`. We are interested `arc-sqlmi` - which contains the binaries that make up the core SQL Server Engine. We can leverage our command from earlier to pull out the Dockerfile as follows:
...
To close off, let's validate this reverse engineering method against the Dockerfile for an Arc container that _isn't_ running the Mariner OS such as [Fluentbit](https://fluentbit.io/) which uses a [distroless base image from gcr with GCC available](https://github.com/GoogleContainerTools/distroless/tree/main/cc), and compare the MCR image against the [Dockerhub version](https://hub.docker.com/r/fluent/fluent-bit/tags?page=1&ordering=last_updated):
...
I've found the Dockerfile reverse engineering technique using Whaler to be fairly robust - and as we saw it also works equally well for other Arc Containers that aren't running the Mariner Base OS.
