...
We looked at how to process Capture files with Auto Loader earlier [here](../event-hub-capture-with-autoloader/). This article presents an alternative approach that allows us to query the same set of `avro` files **in place** using [`KQL`](https://docs.microsoft.com/en-us/azure/data-explorer/kusto/query/) instead - with the added benefit of not having to specify the payload schema or move the data from ADLS.
...
We begin with an existing ADX Database (setup instructions [here](https://docs.microsoft.com/en-us/azure/data-explorer/create-cluster-database-resource-manager)), and create a **external table** on top our Data Lake using the command below:
...
- We specify the `avro` schema (not the `JSON` payload schema) when creating the External Table. This schema isn't likely to change as it's the same for all Event Hubs.
...
And finally, we can query the contents of our Event Hub Capture `JSON` payloads without having to specify the schema, thanks to the [`bag_unpack()`](https://docs.microsoft.com/en-us/azure/data-explorer/kusto/query/bag-unpackplugin) command
